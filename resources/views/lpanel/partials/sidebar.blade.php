<div class="d-none d-md-flex flex-column flex-shrink-0 p-3 text-dark bg-white min-vh-100 border-end sidebar-main">
    ...
    <div class="dropdown border-top pt-3 mt-auto">
        <a href="#" class="d-flex align-items-center text-dark text-decoration-none dropdown-toggle"
            ...
            <img src="{{ asset(Auth::user()?->user_photo ? 'storage/user/' . Auth::user()->user_photo : 'admin/assets/photo_none.png') }}"
                class=" obf-photo rounded-circle">

            <strong class="ms-2">{{ Auth::user()?->user_name ?? 'Misafir Kullanıcı' }}</strong>
        </a>
        <ul class="dropdown-menu text-small shadow" aria-labelledby="dropdownDesktop">
            <li><a class="dropdown-item" href="{{ route('logout') }}"><i class="bi bi-box-arrow-right me-2"></i>Çıkış
                    Yap</a></li>
        </ul>
    </div>
</div>
